...
export default () => {
    ...
    const deleteHandler = (id: any) => {
        myRequest(`/Direction/${id}`, { method: 'DELETE' }).then((res: any) => {
    ...
    const refreshTable = (filter: Filter = { name: null }) => {
        ...
        myRequest("/Direction", { method: 'POST', data: filter }).then((res: any) => {
            setDataSource(res);
            ...
            render: (value, record, index) => {
                return <>
                    <div style={{ userSelect: "none" }}>
                        <Link to={`/direction/edit/${record.id}`}>Изменить </Link>
